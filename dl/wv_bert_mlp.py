...
model: WV_Bert_MLP
======
A class for MLP using Word2Vec and Bert embedding.
配置文件：cmed.dl.wv_bert_mlp.norm.json
"""
...
class WV_Bert_MLP(nn.Module):
    def __init__(self, **config):
        super(WV_Bert_MLP, self).__init__()
        self.config = config
    ...
    def eval_process(self, model, run_mode, data_path):
        ...
        with torch.no_grad():
            ...
            for seq_list, tag_list in DataLoader(**self.config).data_generator(data_path=data_path,
                ...
                #     seq_len = len(y[0])
                #     for i in range(len(y)):
                #         answer = y_true[i*seq_len:(i+1)*seq_len-1]
                #         pred = y_predict[i*seq_len:(i+1)*seq_len-1]
                #         for num in range(len(pred)):
                #             if pred[num] == 'B-Image' and answer[num] != 'B-Image':
                #                 print('\n\n')
                #                 print(seq_list[i])
                #                 print(y_true[i*seq_len:(i+1)*seq_len-1])
                #                 print(y_predict[i*seq_len:(i+1)*seq_len-1])
    ...
    def word_vec(self, data_path, run_mode):
        data_file = '{}data.input.{}'.format(data_path, run_mode)

        # 获得total_examples参数的值(line_num)
        with open(data_file, 'r', encoding='utf-8') as fp:
            input_data = fp.readlines()
            line_num = len(input_data)

        if run_mode == 'train':
            model = Word2Vec(corpus_file=data_file, size=self.embedding_dim, min_count=1)
            model.save('./model/wv_model')
        else:
            # 增量训练
            model = Word2Vec.load('./model/wv_model')
            model.build_vocab(corpus_file=data_file, update=True)  # 更新词汇表
            model.train(corpus_file=data_file, total_examples=model.corpus_count, epochs=model.epochs)
            model.save('./model/wv_model')
