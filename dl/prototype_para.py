...
class Parallel(Init_Class):
    ...
    def forward(self, seq_list):
        ...
        # 方式1：直接拼接各模型输出
        output = torch.cat([mlp_output, bilstm_output, cnn_output, att_output], dim=2)
        output = torch.nn.Dropout(0.5)(output)
        # print('output shape: {}'.format(output.shape))
        output = self.concat_output_to_tag(output)

        # 方式2：模型输出降维至hidden_dim后拼接
        # output = torch.cat([self.linear_output_to_hidden(mlp_output),
        #                     self.rnn_output_to_hidden(bilstm_output),
        #                     self.cnn_output_to_hidden(cnn_output),
        #                     self.att_output_to_hidden(att_output)], dim=2)
        # print('output shape: {}'.format(output.shape))
        # output = self.multi_output_to_tag(output)
    ...
    def test(self, data_path):
        ...
        run_mode = 'test'
        model = torch.load('./model/parallel.ckpt')
